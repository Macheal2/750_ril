...
static void pollSignalRegularly(int* param __unused) {
	...
	//zqy add for do sar operation
	int set_sarvalue = property_get_int32("persist.sys.sar", 0);
	RLOGD("currnt set_sarvalue is:%d,former value is:%d",set_sarvalue,former_sar_value);
	if(former_sar_value ==  set_sarvalue){
		RLOGD("current set sar value is equal to former just return do nothing");
		return;
	}
	RLOGD("set net sar value ");
	
	if(CMRequestSetBodySar(set_sarvalue) == 0){
		RLOGD("currnt time set sar value success if next poll sar value equal to this do nothing");
		former_sar_value = set_sarvalue;
	}
}
...
static void gpsInitATCmd() {
    ...
    if((QCM == curr_modem_info.info.sltn_type) && (curr_modem_info.info.at_version == AT_VERSION_2)) {
		...
		//zqy add
		at_send_command("AT+GPSCFG=\"gnssconfig\",0",NULL);
